...
function inspectItem(monkey, item, toMod) {
  const worryLevel = computeWorryLevel(item, monkey.operation);
  const newWorryLevel = worryLevel % toMod;
...
(async () => {
  ...
  const toMod = MONKEYS.reduce((acc, monkey) => acc * monkey.condition, 1);
  Array.from({ length: 10_000 }).forEach(() => {
    MONKEYS.forEach((monkey) => {
      while (monkey.items.length > 0) {
        inspectItem(monkey, monkey.items[0], toMod);
      }
    });
  });
